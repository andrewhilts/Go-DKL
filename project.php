...
if ($db_found){
      ...
      if ($class_type_array) {
          ...
          if ($ActiveLibrary) {
              ...
              $dd_SQL = "select project_goals.goal_name as 'project goal', lib_goals.goal_class as 'goal class', r1.parent as 'Soft Goal', r1.type as 'contribution', r1.child as 'Action', r2.child as 'Design Decision' from project_goal_class_library_goal as lib_goals INNER JOIN relationships as r1 ON lib_goals.library_goal = r1.parent INNER JOIN relationships as r2 ON r1.child = r2.parent INNER JOIN nodes as n1 on r1.parent = n1.name INNER JOIN nodes as n2 on r1.child = n2.name INNER JOIN nodes as n3 ON r2.child = n3.name INNER JOIN project_goals ON lib_goals.project_goal_id = project_goals.id WHERE n1.type='Soft Goal' AND n2.type='Action' AND n3.type = 'Design Decision' AND project_goals.project_name = '$ActiveProject' ORDER BY `$order_by` $order_dir;";
              $active_dd_SQL = "select project_goals.goal_name as 'project goal', lib_goals.goal_class as 'goal class', r1.parent as 'Soft Goal', r1.type as 'contribution', r1.child as 'Action', r2.child as 'Design Decision' from project_goal_class_library_goal as lib_goals INNER JOIN relationships as r1 ON lib_goals.library_goal = r1.parent INNER JOIN relationships as r2 ON r1.child = r2.parent INNER JOIN nodes as n1 on r1.parent = n1.name INNER JOIN nodes as n2 on r1.child = n2.name INNER JOIN nodes as n3 ON r2.child = n3.name INNER JOIN project_goals ON lib_goals.project_goal_id = project_goals.id WHERE n1.type='Soft Goal' AND n2.type='Action' AND n3.type = 'Design Decision' AND lib_goals.project_goal_id = '$ActiveGoal' ORDER BY `$order_by2` $order_dir2;";
          }
      }
