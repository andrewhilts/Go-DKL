...
function tree_propagate(start, level){
    ...
    for(x=0; x<siblings.length; x++){
        y = siblings[x].value;
        checked = $(siblings[x]).parent().children().children("input[type=checkbox]").attr("checked");
        if(checked){
...
$('#alternatives_analysis_container').find('input[type=checkbox]').change( 
  ...
  switch(level){
    case 2:
        starting_node= $(this).parent().parent().children("ul").children("li").children("select");
        starting_node= starting_node.get(0);
        tree_propagate(starting_node, 3);
        break;
    case 3:
  ...
  same_nodes = $('#alternatives_analysis_container').find('input[type=checkbox][name='+this.name+']');
  if($(this).attr('checked')){
  $(same_nodes).attr('checked', true);
  update_status("node","selected",same_nodes.length);
  $(same_nodes).parent().parent().children("span").removeClass("alternatives_deselected");
  }
  else{
  $(same_nodes).attr('checked', false);
  update_status("node","deselected",same_nodes.length);
  $(same_nodes).parent().parent().children("span").addClass("alternatives_deselected");
  }
